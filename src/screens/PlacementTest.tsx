...
const PlacementTest: React.FC<PlacementTestProps> = ({ onComplete }) => {
  ...
  const getFinalLevel = () => {
    // Logic: If user got at least 2/3 correct in a level, they pass that level
    if (levelScores[Level.B2] >= 2) return Level.B2;
    if (levelScores[Level.B1] >= 2) return Level.B1;
    if (levelScores[Level.A2] >= 2) return Level.A2;
    return Level.A1;
  };
